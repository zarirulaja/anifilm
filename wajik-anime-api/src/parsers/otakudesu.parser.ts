...
import otakudesuExtraParser from "@parsers/extra/otakudesu.extra.parser.js";
...
const otakudesuParser = {
  parseHome(document: HTMLElement): T.IHome {
    ...
    const completedAnimeList: T.ICompletedAnimeCard[] =
      completedAnimeElems?.map((animeEl) => {
  ...
  parseSchedules(document: HTMLElement): T.IScheduleCollection[] {
    ...
    const scheduleList: T.IScheduleCollection[] = scheduleElems.map((scheduleEl) => {
      const title = Text(scheduleEl.querySelector("h2"));
      const animeElems = scheduleEl.querySelectorAll("ul li a");
      const animeList: T.ITextAnimeCard[] = animeElems.map((animeEl) => {
  ...
  parseAllAnimes(document: HTMLElement): T.IAnimeCollection[] {
    const parentElems = document.querySelectorAll(".bariskelom");
    const list: T.IAnimeCollection[] = parentElems.map((letterEl) => {
      const startWith = Text(letterEl.querySelector(".barispenz"));
      const animeElems = letterEl.querySelectorAll(".jdlbar");
      const animeList: T.ITextAnimeCard[] = animeElems.map((animeEl) => {
        const { id, title, otakudesuUrl } = otakudesuExtraParser.parseTextCard(
          animeEl.querySelector("a")!
        );
  ...
  parseOngoingAnimes(document: HTMLElement): T.IOngoingAnimeCard[] {
    ...
    const animeList: T.IOngoingAnimeCard[] = animeElems.map((animeEl) => {
      const animeCard = otakudesuExtraParser.parseOngoingCard(animeEl);
  ...
  parseCompletedAnimes(document: HTMLElement): T.ICompletedAnimeCard[] {
    const animeElems = document.querySelectorAll(".venz ul li");
    const animeList: T.ICompletedAnimeCard[] = animeElems.map((animeEl) => {
      const animeCard = otakudesuExtraParser.parseCompletedCard(animeEl);
  ...
  parseSearchedAnimes(document: HTMLElement): T.ISearchedAnimeCard[] {
    ...
    const animeList: T.ISearchedAnimeCard[] = animeElems.map((animeEl) => {
      const genreElems =
        animeEl.lastElementChild?.previousElementSibling?.previousElementSibling?.querySelectorAll(
          "a"
        )!;
      const genreList = otakudesuExtraParser.parseTextGenreList(genreElems);
  ...
  parseAnimesByGenre(document: HTMLElement): T.IGenreFilteredAnimeCard[] {
    const animeElems = document.querySelectorAll(".page .col-anime");
    const animeList: T.IGenreFilteredAnimeCard[] = animeElems.map((animeEl) => {
  ...
  parseAnimeDetails(document: HTMLElement): T.IAnimeDetails {
    ...
    for (let i = 0; i < headerTitleElems.length; i++) {
      ...
      if (
        !headerTitleEl?.text.toLowerCase().includes("batch") &&
    ...
    const recommendedAnimeList: T.IRecommendedAnimeCard[] = animeElems.map((animeEl) => {
      return {
  ...
  async parseEpisodeDetails(document: HTMLElement, url: string): Promise<T.IEpisodeDetails> {
    ...
    navigationElems.forEach((navigationEl) => {
      ...
      const navigationObj: T.ITextEpisodeCard = {
    ...
    const server: IFormat = {
      ...
      qualityList: serverElems.map((serverEl) => {
        ...
        const serverList: IServer[] = serverElems.map((serverEl) => {
          ...
          const encodedServerId = Buffer.from(JSON.stringify(decodedServerId), "utf-8").toString(
            "base64url"
          );
  ...
  parsePagination(document: HTMLElement): IPagination | undefined {
    function generatePage(el: HTMLElement | null): number | null {
