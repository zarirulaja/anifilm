...
function CompletedContent() {
  ...
  useEffect(() => {
    async function fetchCompleted() {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/anime/completed?page=${pageParam}`);
        const json = await res.json();
        if (json.success) {
          setAnimeList(json.data.animeList || []);
          setPagination(json.data.pagination);
        } else {
          setError(json.error || 'Gagal memuat data completed anime');
        }
      } catch (err) {
        console.error('Completed page error:', err);
  ...
  const handlePageChange = (newPage: number) => {
    router.push(`/completed?page=${newPage}`);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Kumpulan serial anime yang telah tuntas penayangannya.
          </p>
        </div>
      </div>
